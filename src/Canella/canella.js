...
#fad-artist svg, #fad-album svg {
    ...
    let cover, back, title, artist, album, prog, elaps, durr, play, visualizer;
    ...
    function render() {
        ...
        style.innerHTML = styleBase + iconStyleChoices[CONFIG.icons ? 1 : 0];
...
<div id="fad-foreground">
    ...
    <div id="fad-details">
        <canvas id="fad-visualizer"></canvas>
        ...
        visualizer = container.querySelector("#fad-visualizer");
    ...
    let audioSocket;
    let freqData = [];
    let visualizerRedrawRequest;

    function activateVisualizer() {
        audioSocket = new WebSocket("ws://127.0.0.1:6347/audio_hook");
        ...
        audioSocket.onmessage = (event) => {
            ...
            updateVisualizerData(numSamples, channels);
        };
        audioSocket.onclose = (event) => {
            console.log(`Connection to audio server was closed unexpectedly (code=${event.code} reason='${event.reason}')`);
    ...
    function deactivateVisualizer() {
        audioSocket.close();
        if (visualizerRedrawRequest) {
            cancelAnimationFrame(visualizerRedrawRequest);
        }
    }
    function updateVisualizerData(numSamples, channels) {
        let numBins = numSamples / 2;
        if (freqData.length != numBins) {
        ...
        for (let i = 0; i < numBins; i++) {
            ...
            let a = 0.9; //smoothing factor
            freqData[i] = freqData[i] * a + mag * (1.0 - a);
        }
    }
    function renderVisualizer() {
        let ctx = visualizer.getContext("2d");
        let width = visualizer.width = visualizer.offsetWidth;
        let height = visualizer.height = visualizer.offsetHeight;

        ctx.fillStyle = "rgba(0,0,0,0)";
        ctx.fillRect(0, 0, width, height);

        ctx.save();
        ctx.transform(1, 0, 0, -1, 0, height); //make origin bottom-left
        ...
        ctx.beginPath();
        ctx.lineWidth = BAR_WIDTH - SPACING;
        ctx.lineCap = "round";
        ctx.strokeStyle = "#fff";
        for (let i = 0; i < NUM_BARS; i++) {
            let x = SPACING + i * BAR_WIDTH;
            let barHeight = 0;

            let numBins = freqData.length * 0.8; //exclude cutoff
            let binStart = Math.floor(i * numBins / NUM_BARS);
            let binEnd = Math.ceil((i + 1) * numBins / NUM_BARS);
            for (let j = binStart; j < binEnd; j++) {
                let mag = freqData[j];
                //if (mag > barHeight) barHeight = mag;
                barHeight += mag;
            }
            barHeight /= binEnd - binStart;
            ...
            ctx.moveTo(x, 0);
            ctx.lineTo(x, Math.max(1, barHeight * height * 0.5));
        }
        ctx.stroke();
        ctx.restore();
        visualizerRedrawRequest = requestAnimationFrame(renderVisualizer);
    }
